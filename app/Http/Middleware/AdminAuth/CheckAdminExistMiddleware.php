...
class CheckAdminExistMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($admin) {
            return response()->error('Admin already exists', null, 409);
        }
        ...
        if ($user) {
            return response()->error('Email already registered as a user account. Please use a different email.', null, 409);
        }
